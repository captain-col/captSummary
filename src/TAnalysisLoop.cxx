...
/// Set an option and return true if it is valid.  This is called by the event
/// loop command line argument processing code for each "-O [name]=[value]"
/// option found on the command line.  If the command line has "-O [name]"
/// without a value, then the value string will be equal to "".  This must
/// return false if the option was not correctly processed.  Any -O name=value
/// options where name is not explicitly given here will be checked against
/// all the modules, and the value sent to module::Configure() in the case of
/// a match.
...
/// Called for each event inside the event loop, and returns true if the event
/// should be saved to the output file.  If the remainder of the current file
/// should be skipped, this should through the ENextEventLoopFile exception.
...
/// Initialize() for general initialization.  This method will be called once
/// for each input file.
